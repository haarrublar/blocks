...
async function updatePlayerStatus(username, isConnected, type) {
	...
	try {
		await axios.post("http://127.0.0.1:8000/bot/players/", payload, {
			timeout: 2000,
		});
		console.log(
			`[Django Update] ${username}: ${isConnected ? "Online" : "Offline"}`,
		);
	} catch (error) {
...
async function chat(sessionId, username, message) {
	...
	try {
		await axios.post("http://127.0.0.1:8000/bot/chat/", payload, {
			timeout: 10000,
		});
...
async function buildProgress(
	...
	try {
		...
		if (taskId) {
			console.log(
				`[Database Success] Task #${taskId} created for ${playerUsername}`,
			);
			return taskId;
		} else {
			console.log(
				`[Database Warning] Record saved, but no ID returned. Check Serializer fields.`,
			);
			return null;
		}
	} catch (error) {
