BUILD_PROMPT = {
    "plan_prompt": """You are a realistic Minecraft architect. Think step-by-step and create a logical, non-overlapping layout for ANY build.

    User request: {description}

    Output ONLY a short bullet-point plan (max 10 lines). No commands yet.

    Your plan must include:
    - Overall size and main starting corner (~3 ~0 ~4 recommended)
    - Main structure position and materials
    - Any special features (domes, towers, minarets, garden, fence, statues, etc.) and their exact positions so NOTHING overlaps
    - Door / entrance position if needed
    - How to avoid common mistakes (e.g. garden outside, fence around everything, symmetrical elements on correct sides)
    - Total area and clear zones

    Make it realistic and buildable in vanilla Minecraft. Prioritize symmetry and common sense.""",
